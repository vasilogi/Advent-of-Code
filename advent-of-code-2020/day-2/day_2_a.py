...
for index, row in df.iterrows():
    ...
    data = row.to_dict()
    # check if the character exist in the password
    if data['character'] in data['password']:
        # count the frequency of the character in the password
        counter = data['password'].count(data['character'])
        # inspect if it is bounded as desired
        if counter >= int(data['minChar']) and counter <= int(data['maxChar']):
            validPassCounter += 1
            continue
